"""Ground-truth true-bucket regressions from Aaron's listened samples.
...
def test_reverb_keys_control_ignores_generic_reverse_fx_candidate() -> None:
    """A warm keys/pluck loop must not be pulled to generic Reverse/Glitch FX."""
    final = DecisionCoreV2().apply_eligibility(
        raw(
            "Instruments/Instrument Loops/Loops",
            "Instruments",
            ...
            candidates=[
                candidate("Instruments/Instrument Loops/Loops", 8),
                candidate("FX/Structural and Transitional FX/Reverses and Tails/Generic Reverse/Long FX", 9),
                candidate(
                    "FX/Digital Mechanical Industrial Transport/Glitches and Stutters/Generic Glitch/Long FX", 10
                ),
            ],
        ),
        eligibility("pitched_music_loop", ("Instruments", "_TO_REVIEW"), "Instruments/Instrument Loops/Loops"),
...
def test_metal_percussion_control_prefers_drums_over_generic_reverse_fx() -> None:
    """Metallic percussion loops should not be stolen by generic Reverse/Tail FX candidates."""
    final = DecisionCoreV2().apply_eligibility(
        raw(
            "Instruments/Instrument Loops/Loops",
            "Instruments",
            ...
            candidates=[
                candidate("Instruments/Instrument Loops/Loops", 9),
                candidate("Drums/Drum Loops/Loops", 10),
                candidate("FX/Structural and Transitional FX/Reverses and Tails/Reverse Cymbal/Long FX", 11),
            ],
        ),
        eligibility("pitched_music_loop", ("Instruments", "_TO_REVIEW"), "Instruments/Instrument Loops/Loops"),
    )
...
def test_pitched_percussion_eligibility_broadens_to_drums_not_tonal_fx() -> None:
    """A measured pitched_percussion_loop must not be overruled by close riser/build FX."""
    final = DecisionCoreV2().apply_eligibility(
        raw(
            "Instruments/Instrument Loops/Loops",
            "Instruments",
            ...
            candidates=[
                ...
                candidate("Drums/Drum Loops/Loops", 18),
            ],
        ),
        eligibility("pitched_percussion_loop", ("Drums", "_TO_REVIEW"), "Drums/Drum Loops/Loops"),
    )
    assert final.final_top in {"Drums", "Instruments", "_TO_REVIEW"}
    assert final.folder_path != "FX/Structural and Transitional FX/Risers and Builds/Synth Riser/Long FX"
