#!/usr/bin/env python3
"""Run Aaron Sound Sorter against random real sample folders, logs and real sorting.

This tool samples folders under a sample-library root, picks a few audio files from
each selected folder, runs the sorter one file at a time, keeps manifests,
summaries, stdout, CSV reports, AND copies the actual sorted audio files into a
consolidated, organized master folder.

It does not copy audio into the report zip to keep the upload-back payload small.
"""
...
def run_sorter_one_file(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="aaron_random_folder_panel_") as tmp:
        ...
        # Capture and merge the actual sorted sound files into the main output folder
        if real_sort_root and status == "pass":
            for item_path in sort_output.rglob("*"):
                if item_path.is_file():
                    # Skip root logs to prevent them from constantly overwriting each other
                    if item_path.name in ("Aaron_Sorted_Sounds_manifest.csv", "Aaron_Sorted_Sounds_summary.txt"):
                        continue
                    rel_path = item_path.relative_to(sort_output)
                    dest_file_path = real_sort_root / rel_path
...
def main() -> int:
    ...
    # Establish the master target directory for the real sorted sound files
    real_sort_root = (project_root / args.out_root / f"sorted_{stamp}").resolve()
    real_sort_root.mkdir(parents=True, exist_ok=True)
    ...
    # This zips up ONLY report_dir, completely ignoring the big audio files in real_sort_root
    upload_zip = shutil.make_archive(str(report_dir / "random_folder_log_panel_upload_back"), "zip", report_dir)
    print(f"\nReport folder: {report_dir}")
    print(f"Real sorted sounds folder: {real_sort_root}")
