...
Defaults are resolved from the repository containing this script. Pass one or
more --search-root values to search an external sample library.
...
DEFAULT_SEARCH_ROOTS = [DEFAULT_PROJECT_ROOT]
DEFAULT_DEST = DEFAULT_PROJECT_ROOT / "tests/acceptance/locked_smoke_v1/samples"
DEFAULT_MANIFEST = DEFAULT_PROJECT_ROOT / "tests" / "acceptance" / "locked_smoke_v1" / "expected_results.json"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help=(f"Root folder to search recursively. Can be used more than once. Default: {DEFAULT_SEARCH_ROOTS[0]}"),
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
