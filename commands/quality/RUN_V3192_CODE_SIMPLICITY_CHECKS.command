...
REPORT_ROOT="$PROJECT_ROOT/reports/v3192_code_simplicity_checks"
...
for sample_path in "$SAMPLE_DIR"/*.wav; do
  base_name="$(basename "$sample_path")"
  label="$(echo "$base_name" | sed 's/_.*//')"
  case "$base_name" in
    drum_loop*) label="drum_loop" ;;
    vocal_loop*) label="vocal_loop" ;;
  esac
...
(cd "$RUN_DIR" && zip -qr "v3192_code_simplicity_results_$STAMP.zip" . -x '*.zip' '*.wav' '*.aif' '*.aiff' '*/._*' '._*' '*/.DS_Store' '.DS_Store' '*/__MACOSX/*' '__MACOSX/*')
...
echo "Upload-back ZIP: $RUN_DIR/v3192_code_simplicity_results_$STAMP.zip"
